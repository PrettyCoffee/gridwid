...
const rgbToOklch = (...rgb: ColorValue["color"]): ColorValue["color"] => {
  ...
  const linearRgb = {
    r: r <= 0.040_45 ? r / 12.92 : Math.pow((r + 0.055) / 1.055, 2.4),
    g: g <= 0.040_45 ? g / 12.92 : Math.pow((g + 0.055) / 1.055, 2.4),
    b: b <= 0.040_45 ? b / 12.92 : Math.pow((b + 0.055) / 1.055, 2.4),
  }
  ...
  const fx =
    xNorm > 0.008_856 ? Math.pow(xNorm, 1 / 3) : 7.787 * xNorm + 16 / 116
  const fy =
    yNorm > 0.008_856 ? Math.pow(yNorm, 1 / 3) : 7.787 * yNorm + 16 / 116
  const fz =
    zNorm > 0.008_856 ? Math.pow(zNorm, 1 / 3) : 7.787 * zNorm + 16 / 116
