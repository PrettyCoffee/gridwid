  ...
  return (
    <input
      ...
      className={cn(
        ...
        "inline-flex h-10 justify-center truncate bg-transparent px-2.5 text-text",
        className
      )}
    />
  )
  ...
  return (
    <textarea
      ...
      className={cn(
        ...
        "block w-full resize-none bg-transparent px-3 py-1 text-text",
        className
      )}
    />
  )
...
export const EditorMarkdown = ({ field, ...props }: InputProps) => {
  ...
  return (
    <MDEditor
      ...
      onFocus={() => setHasFocus(true)}
      onBlur={() => setHasFocus(false)}
      onSave={input.didChange ? editor.save : undefined}
      inputClassName={cn(
        inputBorder({
          status: hasError ? "error" : isEditing ? "editing" : "default",
        })
